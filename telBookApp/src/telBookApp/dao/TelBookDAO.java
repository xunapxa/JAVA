...
//		System.out.println("DAO update");
		...
		String sql = "UPDATE telbook SET name =?, age =?, address =?, phone =? WHERE id=?";
		try {
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, telBook.getName());
			psmt.setInt(2, telBook.getAge());
...
//		System.out.println("DAO getListAll");
		...
		try {
			...
			while(rs.next()) { //다음 배열을 읽어보고 존재하지않으면(false) 빠져나옴
...
//		System.out.println("DAO getOne");
	...
	@Override
	public List<TelBook> serchKeyword(int num, String keyword) {
		List<TelBook> result = new ArrayList<TelBook>();
		Connection conn = DBConn.getConnection();
		PreparedStatement psmt = null;
		ResultSet rs = null;
		String sql = "";
